...
export default {
    ...
    async getAllMeasurements(context) {
        console.log("getAllMeasurements");
        const measurements = context.getters.getMeasurements;
    ...
    async getAMeasurement(context, payload) {
        const measurements = context.getters.getmeasurements;
        const measurement = measurements.filter(m => m.id == payload.measurement_id);
        if (measurement.length) return measurement[0];

        try {
            const response = await axios.get(
                context.rootGetters.getUrl + `api/admin/measurements/${payload.measurement_id}`,
                {
                    headers: {
                        ContentType: "application/json"
                    }
    ...
    async updateCurrentMeasurement(context, payload) {
        try {
            ...
            if (response.status == 200 && response.data) {
                const measurements = context.getters.getMeasurements;
                const measurementIndex = measurements.findIndex(m => m.id == payload.measurement_id);
