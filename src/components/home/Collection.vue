<template>
  <div>
		<div 
			...
			class="bg-no-repeat bg-cover bg-center w-96 h-96"
		></div>
		<div class="sm:p-4 lg:p-0 flex flex-col justify-center items-center w-96 sm:my-10">
			<h1 class="font-extrabold font-serif text-3xl md:text-5xl text-gray-800 sm:mt-5 md:mb-5 w-full">
				{{ title }}
			</h1>
			<p class="font-sans text-lg md:text-2xl font-light text-gray-800 mb-10 w-full">
				{{ description }}
			</p>
			<div v-if="isAdmin" class="flex flex-row justify-left">
				<div class="p-2 w-11/12 md:w-3/4 mx-0" style="width:150px;">
					<button
...
export default {
  ...
  methods: {
    ...
    async deleteCollection() {
      ...
      const status = await this.$store.dispatch("collections/deleteCollection", {
